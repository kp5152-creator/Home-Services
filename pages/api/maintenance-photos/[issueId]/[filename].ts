...
import { readMaintenancePhotoAsset } from "@/services/database";
...
export default async function handler(request: NextApiRequest, response: NextApiResponse) {
  ...
  const issueId = String(request.query.issueId ?? "");
  const filename = String(request.query.filename ?? "");
  const extension = path.extname(filename).toLowerCase();

  if (!issueId || !filename || !allowedExtensions.has(extension)) {
    response.status(400).json({ message: "Invalid photo request" });
    return;
  }

  const photo = await readMaintenancePhotoAsset(issueId, filename);
